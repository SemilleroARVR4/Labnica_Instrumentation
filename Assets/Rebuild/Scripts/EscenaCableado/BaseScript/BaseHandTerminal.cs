...
public class BaseHandTerminal : MonoBehaviour
{
    ...
    [SerializeField] private Text mText1; //Titulo
    [SerializeField] private Text mText2; //Renglon 1
    [SerializeField] private Text mText3; //Renglon 2
    [SerializeField] private Text mText4; //Renglon 3

    [SerializeField] private TransmitterScript mTransmisor;

    //Rango maximo y minimo inicial que tiene el handTerminal.
    public double mRangoMax { get; set; } = 3;
    ...
    private bool conModificacion = false;
    ...
    private void ActualizarDato(string num) //En caso de querer modificar el LVR o UVR
    {
        if (conModificacion == true)
        {
    ...
    private void EjecutarBtnsOn(string opc)
    {
        switch (opc)
        {
            ...
            case "<": //Si deseo volver al menu anteior...
                ...
                //Importante: Para el desplazamiento entre menus uso de referencia el texto que esta en algun renglon.
    ...
    private void ActualizarLVRoUVR()
    {
        if (conLVRUVR == false) // Para LVR
        {
            ...
            {
                mRangoMin = Convert.ToSingle(numact);
                mRangoMin = System.Math.Round(mRangoMin, 2);
            }
            numact = string.Empty;
            conLVRUVR = true;
        }
        else // PARA UVR
        {
            ...
            {
                mRangoMax = Convert.ToSingle(numact);
                mRangoMax = System.Math.Round(mRangoMax, 2);
            }
            numact = string.Empty;
            conLVRUVR = false;
        }
